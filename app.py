...
if not api_key:
    st.warning("⚠️ APIキーが未設定よ。Secretsを確認して。")
    if prompt := st.chat_input("（デモ）何か入力して..."):
        st.chat_message("user").write(prompt)
        st.chat_message("assistant").write("タダヒロ、APIキーがあれば本物の知性で答えられるわ。")
else:
    try:
        genai.configure(api_key=api_key)
        # 最も安定している 'gemini-pro' を指定
